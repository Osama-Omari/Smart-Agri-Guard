...
import 'package:smart_agri_guard/features/shared/widgets/admin_report_card.dart';
...
class GreenhousesReportsScreen extends StatefulWidget {
  const GreenhousesReportsScreen({super.key});

  @override
  _GreenhousesReportsScreenState createState() => _GreenhousesReportsScreenState();
}

class _GreenhousesReportsScreenState extends State<GreenhousesReportsScreen> {

  void _loadData(){
    AppCubit.get(context).getGreenhousesNotifications();
  }
  @override
  void initState() {
  ...
  void _markAllRead() async{
    ...
    notificationsIDs = await AppCubit.get(context).greenhousesNotifications.map((e) => e.id!).toList();
                        ...
                        child: Padding(
                          ...
                          child: ListView(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 20),
                              if (state is GetGreenhousesNotificationsLoadingState)
                                const Center(
                                  child: CircularProgressIndicator(),
                                )
                              else if (AppCubit.get(context).greenhousesNotifications.isEmpty)
                                const Center(
                                  child: Text(
                                ...
                                ...AppCubit.get(context).greenhousesNotifications.map((r) {

...
class _AddManagerScreenState extends State<AddManagerScreen> {
  ...
  void _save() {
    ...
    final List<String> greenhouseIds =
    _selectedGreenhouses
        .map((g) => g.Id ?? '')
        .where((id) => id.isNotEmpty)
        .toList();
    AppCubit.get(context).addManager(context, _fullName.text.trim(), _username.text.trim(), _password.text, greenhouseIds);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<AppCubit, AppStates>(
        listener: (context, state) {
          if (state is GetGreenhousesWithoutManagerSuccessState) {
            final cubit = AppCubit.get(context);

            setState(() {
              _greenhouses = cubit.greenhousesWithoutManager
                  .map((g) => g.name ?? '')
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: CustomMultiSelectDropdown<GreenhouseModel>(
                                    title: 'Select Greenhouses',
                                    hintText: 'Tap to choose greenhouses (optional)',
                                    items: AppCubit.get(context).greenhousesWithoutManager,           // ✅ List<GreenhouseModel>
                                    selectedItems: _selectedGreenhouses,        // ✅ Set<GreenhouseModel>
                                    labelBuilder: (g) => g.name ?? '',           // ✅ DISPLAY NAME
                                    onSelectionChanged: (newSelected) {
                                      setState(() {
                                        _selectedGreenhouses = newSelected;
                                      });
                                    },
                                  ),
                                ),
